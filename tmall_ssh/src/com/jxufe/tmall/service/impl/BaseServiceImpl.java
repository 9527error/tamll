...
public class BaseServiceImpl  extends ServiceDelegateDAO implements BaseService {
    ...
    @Override
    public Object get(Class clazz, int id) {//这个好处在于，可以查询别的clazz的bean,比如Category不在局限于查询Category
	...
	public List listByParent(Object parent) {
		//和上面的list区别在于提供了parent,即多了一个筛选条件，查询的bean中需要有parent对象
		//例如，在Property类中调用此方法，传入的是一个Category对象
	...
	public int total(Object parentObject) {
		...
		String sqlFormat = "select count(*) from %s bean where bean.%s = ?";//hql查询和SQL不一样的地方，hql可以用bean来查询，即一个对象
    ...
    public List list(Object... pairParms) {
        ...
        Set<String> ks = m.keySet();//得到所有key的集合
        for (String key : ks) {
            if(null==m.get(key))
